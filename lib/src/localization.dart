...
class Localization {
  ...
  String _replaceNamedArgs(String res, Map<String, String>? args) {
    if (args == null || args.isEmpty) return res;
    args.forEach((String key, String value) => res = res.replaceAll('{$key}', value));
    return res;
  }
  ...
  String plural(
    ...
    // automatic replacement for count
    namedArgs ??= {};
    if (!namedArgs.containsKey('count')) {
      namedArgs['count'] = formattedValue;
    }

    if (name != null) {
      namedArgs[name] = formattedValue;
    }
    res = _replaceNamedArgs(res, namedArgs);
  ...
  String _resolvePlural(String key, String subKey) {
    ...
    final tagUnderscore = '${key}_$subKey';

    var resource = _resolve(tag, logging: false, fallback: false);
    if (resource != tag) return resource;

    resource = _resolve(tagUnderscore, logging: false, fallback: false);
    if (resource != tagUnderscore) return resource;

    if (subKey == 'other') {
      return _resolve(tag);
    }

    final tagOther = '$key.other';
    resource = _resolve(tagOther, logging: false, fallback: false);
    if (resource != tagOther) return resource;

    final tagOtherUnderscore = '${key}_other';
    resource = _resolve(tagOtherUnderscore, logging: false, fallback: false);
    if (resource != tagOtherUnderscore) return resource;
